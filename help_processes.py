...
def training_process(folder):
    regime = "ForTest"
    for file_name in sorted(os.listdir(regime + "/Texts")):
        print(file_name)
        print("-------")
        source_file_name = regime + "/Texts/" + file_name
        f = open(source_file_name, encoding='utf-16')
        s = f.read()
        doc = parsing_process(s)
        anaphors = find_anaphors(doc)
        if len(anaphors) > 0:
            res_text_file = folder + "/texts/" + file_name
            shutil.copy2(source_file_name, res_text_file)
            res_mark_file = folder + "/marks/" + file_name
            f = open(res_mark_file, encoding='utf-16', mode='w+')
            pref = ''
            for j in anaphors:
                print([i.text.upper() if i.start == j[0].start else (i.text if i not in j[2] else i.text + ' [' + str(j[2].index(i)) + ']') for i in j[3]])
                print(j[0].start)

                decision = list(map(int, input('decision: ').split()))
                if len(decision) == 0:
                    f.write(pref + str(j[0].start) + ' -')
                else:
                    f.write(pref + str(j[0].start) + ' ' + ' '.join([j[2][i].lemma for i in decision]))
                if pref == '':
                    pref = '\n'

        f.close()
        os.remove(regime + "/morph/" + file_name)
        os.remove(regime + "/Texts/" + file_name)
        ...
        for word in sen.tokens:
            ...
            if word.pos == "PRON":
                if "Person" in  word.feats and word.feats["Person"] == '3':
                    ...
                    result.append([word, candidates_vec, candidates, prevprev + prev + sen.tokens])
